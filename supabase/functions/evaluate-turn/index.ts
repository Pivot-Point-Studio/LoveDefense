...
import { retrieveCoachingKnowledge } from "./ragRetriever.js";
...
type TurnResponse = { result: unknown; model: string; retrieval?: Record<string, unknown> | null };
const REQUEST_CACHE = new Map<string, { createdAt: number; response: TurnResponse }>();
...
function storeResponse(key: string, response: TurnResponse) {
...
retrievedCoachingContext는 서버가 현재 프로필, 갈등, 최근 대화와 사용자 입력을 기준으로 검색한 프로젝트 큐레이션 코칭 지식이다. 점수 자체를 올리기 위한 정답으로 간주하지 말고, 문맥에 맞는 항목만 weaknesses와 suggestedBetterResponse의 근거로 사용하라. 검색 지식을 사용자·상대의 실제 사실로 단정하거나 출처 문구를 그대로 복사하지 마라.
suggestedBetterResponse는 검색된 지식 중 현재 문맥에 맞는 전략을 구체적으로 반영하되 최근 추천과 다른 한국어 답변으로 최대 두 문장, 200자 이내다. 내부 반응 지침이나 행동 설명을 대사처럼 쓰지 마라.
...
retrievedCoachingContext가 입력에 있어도 이는 평가와 추천 표현용 참고 지식일 뿐 상대방이 알고 있는 사실이나 상대방 대사가 아니다. partnerDialogue에서 RAG, 검색, 코칭 카드, 출처를 언급하거나 코치처럼 말하지 마라.
...
serve(async (request) => {
  ...
  try {
    ...
    const retrieval = requestType === "generate_partner_dialogue" ? null : retrieveCoachingKnowledge(input, { topK: 3 });
    const promptInput = retrieval ? { ...input, retrievedCoachingContext: retrieval.promptContext } : input;
    const response = requestType === "evaluate_user_input"
      ? await callOpenAI(buildEvaluationPrompt(promptInput), EVALUATION_SCHEMA, "turn_evaluation")
      : requestType === "evaluate_and_generate_turn"
        ? await callOpenAI(buildCombinedPrompt(promptInput), COMBINED_SCHEMA, "turn_with_partner_dialogue")
        : await callOpenAI(buildDialoguePrompt(promptInput), DIALOGUE_SCHEMA, "partner_dialogue");
    const enrichedResponse = { ...response, retrieval: retrieval?.metadata ?? null };
    storeResponse(cacheKey, enrichedResponse);
    return new Response(JSON.stringify({ ...enrichedResponse, provider: "openai", requestId: input.requestId }), { headers: { ...cors, "Content-Type": "application/json" } });
  } catch (error) {

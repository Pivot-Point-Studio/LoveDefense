...
export async function getDialogueDiversityState() { if (!supabase) return result(null); const r = await supabase.from("profiles").select("dialogue_diversity").eq("user_id", getCurrentUserId()).maybeSingle(); return result(r.data?.dialogue_diversity ?? null, r.error) }
export async function saveDialogueDiversityState(dialogueDiversity) { if (!supabase) return result(null, { message: "not configured" }); const r = await supabase.from("profiles").update({ dialogue_diversity: dialogueDiversity }).eq("user_id", getCurrentUserId()).select("dialogue_diversity").maybeSingle(); return result(r.data?.dialogue_diversity ?? null, r.error) }

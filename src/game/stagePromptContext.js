...
export function buildDialoguePayload(input, evaluation, context) {
  ...
  const coachingProfile = (profile) => profile ? { mbti: profile.mbti ?? "", tendency: profile.tendency ?? "", expressionStyle: profile.expressionStyle ?? profile.expression ?? "", desiredResponse: profile.desiredResponse ?? "", conflictStyle: profile.conflictStyle ?? "", relationshipNeed: profile.relationshipNeed ?? "" } : null
  return { stageNumber: context.stage.stageNumber, turnNumber: context.turn.turnNumber, maxTurns: context.stageDifficulty.turnCount, scenarioId: scenario.id, scenarioTitle: scenario.title, contextSummary: scenario.contextSummary, location: scenario.location, timeContext: scenario.timeContext, conflictCause: scenario.conflictCause, hiddenEmotion: scenario.hiddenEmotion, hiddenNeed: scenario.hiddenNeed, userRole: scenario.userRole, partnerRole: scenario.partnerRole, stageGoal: scenario.goal, resolutionState: context.resolutionState ?? "unresolved", stageSensitivity: getStageSensitivity(context.stage.stageNumber), gameState: context.gameState, userProfile: coachingProfile(context.userProfile), partnerProfile: coachingProfile(context.partnerProfile ?? context.character), character: context.character, attachmentType: context.stage.attachmentType, communicationAxes: context.stage.communicationAxes, userInput: input, evaluation, reactionDirection: evaluation.reactionDirection, recentMessages, conversationHistory: recentMessages, recentPartnerDialogues: patternContext.recentPartnerResponses, recentSuggestedResponses: (context.diversityState?.recentSuggestedResponses ?? context.previousAdvice ?? []).slice(-10), previousStageSummary: context.previousStageSummary ?? "", patternContext, recentDialogueEndings: patternContext.recentEndings }
}
